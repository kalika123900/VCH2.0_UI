...
class ComposeEmailForm extends React.Component {
  ...
  handleSuccess = () => {
    const contentBlock = convertFromRaw(content);
    const editorState = EditorState.createWithContent(contentBlock);
    this.setState({ emailContent: '', files: [], subject: '', editorState });
  }

  handleSend = (subject, emailContent, files) => {
    const { sendEmail } = this.props;
    this.setState({ loading: true })
    sendEmail(subject, emailContent, files, this.handleSuccess);
  };
          ...
          {loading ?
            ...
            <div className={css.buttonArea}>
              ...
              <Button
                ...
                type="button"
                disabled={!subject}
                onClick={() => this.handleSend(subject, emailContent, files)}
              >
                Send&nbsp;
